...
//----- sequelize settings ------//
const sequelize = new Sequelize('db_bull', 'adlenkhelladi', 'AdlenLoeke-209', {
    host: 'localhost',
    dialect: 'postgres',
  define: {
	...
	//Messages.create
				...
				Messages.create({
			 	...
			 	body: req.body.body
				   ...
				   res.redirect('page2'); //which redirects to the secondpage
})




app.get("/page2", (req,res)=> { //you need to get the secondpage, you user asks for it
	Messages.findAll({raw : true}).then((values =>{			
				//console.log(values)				

		 res.render('page2', { posted: values} ); //renders on the same page itself
	})
	);
